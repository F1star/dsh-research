...
import {
  ...
  type RegisterResearchPaperResult,
  type ResearchDocumentObservation,
  type ResearchPaperRecord,
  type ResearchSourceVersion,
  type UpdateResearchAliasResult,
} from '../research-library/index.ts'
import type {} from '@deepseek-ai/dsh-system-prompt'
import { defineTool } from '@deepseek-ai/dsh-tools'
import type { GenericCallView, InferValue, JsonValue } from '@deepseek-ai/dsh-tools'
...
type RuntimeCoverage = 'metadata-only' | 'readable' | 'needs-ocr' | 'reimport-required' | 'parser-mismatch'

const ORIGIN_SCHEMA = { type: 'string', enum: ['declared', 'parser', 'source-derived'] } as const
...
type ProjectedPaper = InferValue<typeof PAPER_SCHEMA>
type RegisterOutput = InferValue<typeof REGISTER_OUTPUT_SCHEMA>
type ListOutput = InferValue<typeof LIST_OUTPUT_SCHEMA>
type GetOutput = InferValue<typeof GET_OUTPUT_SCHEMA>
type AliasOutput = InferValue<typeof ALIAS_OUTPUT_SCHEMA>
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args): Promise<RegisterOutput> {
      ...
      const externalIds = [
        ...(args.doi === undefined ? [] : [{ kind: 'doi' as const, value: args.doi, origin: 'declared' as const }]),
        ...(args.arxiv_id === undefined
          ? []
          : [{ kind: 'arxiv' as const, value: args.arxiv_id, origin: 'declared' as const }]),
      ]
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args): Promise<AliasOutput> {
      const paperId = parsePaperId(args.paper_id)
      let result: UpdateResearchAliasResult
      if (args.kind === 'source') {
...
function runtimeDocumentInput(document: ResearchDocument, sourceAlias: string | undefined) {
  return {
...
function projectRegistration(ctx: Context, result: RegisterResearchPaperResult, config: ResolvedConfig): RegisterOutput {
...
function projectAlias(ctx: Context, result: UpdateResearchAliasResult, config: ResolvedConfig): AliasOutput {
  switch (result.status) {
    case 'updated':
    case 'unchanged': {
      const all = ctx.researchLibrary.list()
...
function projectPaper(
  ...
  possibleDuplicateIds: readonly ReturnType<typeof ResearchPaperId>[],
  ...
  const coverage = paperCoverage(
    paper.sourceVersions.map(source => runtimeCoverage(ctx, source)),
    paper,
  )
  ...
  if (truncated) budget.noteTruncation()
  return {
    paper_id: paper.id,
    metadata: {
...
function projectSource(ctx: Context, source: ResearchSourceVersion, budget: ProjectionBudget): InferValue<typeof SOURCE_SCHEMA> {
  ...
  const truncated = aliases.truncated
    ...
    || projectedObservations.some(observation =>
      observation.document_title_truncated === true
...
function projectObservation(
  ...
  budget: ProjectionBudget,
): InferValue<typeof OBSERVATION_SCHEMA> {
...
function runtimeCoverage(ctx: Context, source: ResearchSourceVersion): Exclude<RuntimeCoverage, 'metadata-only'> {
  const retained = ctx.researchDocuments.peek(source.documentId)
  if (retained === undefined) return 'reimport-required'
  if (retained.extraction.text === 'none') return 'needs-ocr'
  const observedParser = source.observations.some(observation =>
    observation.parserId === retained.parser.id && observation.parserVersion === retained.parser.version)
  return observedParser ? 'readable' : 'parser-mismatch'
}

function paperCoverage(
  sourceCoverage: readonly RuntimeCoverage[],
  paper: ResearchPaperRecord,
): RuntimeCoverage {
  if (paper.acquisitionState === 'metadata-only') return 'metadata-only'
  for (const status of ['readable', 'needs-ocr', 'parser-mismatch'] as const) {
    if (sourceCoverage.includes(status)) return status
  }
...
class ProjectionBudget {
  ...
  private sourcesRemaining: number
  private nestedRemaining: number
  truncated = false
  ...
  sources<T>(values: readonly T[]): { values: readonly T[]; truncated: boolean } {
  ...
  collection<T>(values: readonly T[]): { values: readonly T[]; truncated: boolean } {
...
function duplicateMap(
  papers: readonly ResearchPaperRecord[],
): Map<ReturnType<typeof ResearchPaperId>, ReturnType<typeof ResearchPaperId>[]> {
  const groups = new Map<string, ReturnType<typeof ResearchPaperId>[]>()
  ...
  const duplicates = new Map<ReturnType<typeof ResearchPaperId>, ReturnType<typeof ResearchPaperId>[]>()
  for (const group of groups.values()) {
    if (group.length < 2) continue
    for (const paperId of group) duplicates.set(paperId, group.filter(candidate => candidate !== paperId))
  }
  return duplicates
}

function searchablePaper(paper: ResearchPaperRecord): string {
  return [
...
function normalizedQuery(value: string, maximum: number): string {
  const query = comparableText(value).replace(/\s+/gu, ' ')
  if (query.length === 0) throw new Error('query must be a non-empty string')
  if (query.length > maximum) throw new Error(`query must be at most ${maximum} normalized characters`)
  return query
}
...
function parsePaperId(value: string): ReturnType<typeof ResearchPaperId> {
...
function parseSourceVersionId(value: string): ReturnType<typeof ResearchSourceVersionId> {
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(value)) {
...
function resolveConfig(config: Config = {}): ResolvedConfig {
  const resolved = {
    ...
    maxSourcesPerResult: positiveSafeInteger(
      'maxSourcesPerResult', config.maxSourcesPerResult ?? DEFAULT_MAX_SOURCES_PER_RESULT,
    ),
    maxNestedItemsPerResult: positiveSafeInteger(
      'maxNestedItemsPerResult', config.maxNestedItemsPerResult ?? DEFAULT_MAX_NESTED_ITEMS_PER_RESULT,
    ),
