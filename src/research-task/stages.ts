 ...
 */
export function taskStages(kind: ResearchTaskKind): readonly ResearchTaskStage[] {
...
export function inspectStage(
  stage: ResearchTaskStage, kind: ResearchTaskKind, question: ResearchQuestionRecord,
  sources: readonly ResearchTaskSource[], papers: readonly ResearchPaperRecord[], report: () => ResearchReportResult,
  comparisonOutcome?: 'protocol' | 'not-comparable',
): StageInspection {
  const issues: string[] = []
  const artifacts: ResearchTaskArtifacts = { evidenceIds: [], claimIds: [], claimReviewIds: [], observationIds: [],
    observationReviewIds: [], comparisonProtocolIds: [], synthesisIds: [] }
  const result = (basis: unknown, refs: ResearchTaskArtifacts = artifacts): StageInspection => ({
    issues, basisDigest: `sha256:${createHash('sha256').update(JSON.stringify(basis)).digest('hex')}` as ResearchTaskCheckpoint['basisDigest'],
    artifacts: refs,
  })
  if (stage === 'acquisition') {
    if (sources.length === 0) issues.push('Select at least one imported paper source.')
    const paperIds = new Set(sources.map(source => source.paperId))
    if (kind === 'single-paper' && paperIds.size !== 1) issues.push('Single-paper reading requires exactly one paper.')
    if (kind === 'method-comparison' && paperIds.size < 2) issues.push('Method comparison requires at least two papers.')
  ...
  if (stage === 'review') {
    for (const [index, claim] of claims.entries()) {
      if (latestClaimReviews[index] === undefined) issues.push(`Researcher review is required for claim ${claim.id}.`)
    }
    for (const [index, observation] of observations.entries()) {
      const state = states[index]
      assert(state !== undefined)
      if (state.review === null) issues.push(`Researcher review is required for result ${observation.id}.`)
  ...
  if (stage === 'comparison') {
    if (comparisonOutcome === undefined) issues.push('Record a comparison protocol or explicitly explain why results are not comparable.')
    if (comparisonOutcome === 'protocol' && protocols.length === 0) issues.push('A current protocol over researcher-accepted results is required.')
  ...
  if (stage === 'synthesis') {
    if (syntheses.length === 0) issues.push('Record a current synthesis, including qualifications or open questions where evidence is insufficient.')
