...
import type {} from '../research-workspace/types.ts'
import type {} from '@deepseek-ai/dsh-client-ui-sidebar/client'
import { WorkspaceController } from './controller.ts'
import { Workspace, type WorkspaceInjected } from './Workspace.tsx'
import { Observations, type ObservationsInjected } from './Observations.tsx'
import { Tasks, type TasksInjected } from './Tasks.tsx'
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    'research.workspace.observations': { kind: 'single'; scope: 'root'; owner: Record<never, never> }
    /** Task progress and explicit recovery inside the research workspace. */
    'research.workspace.tasks': { kind: 'single'; scope: 'root'; owner: { openSource: (source: ResearchWorkspaceTaskSource) => void } }
  }
}
...
export function apply(ctx: ClientContext): void {
  const controller = new WorkspaceController({
    createQuestion: async (reviewer, title, question) =>
      unwrap(await ctx.remote.researchWorkspace.createQuestion(reviewer, title, question)),
    ...
    taskSources: async (id, revision, checkpoint, offset) =>
      unwrap(await ctx.remote.researchWorkspace.taskSources(id, revision, checkpoint, offset)),
    ...
    observationChoices: async (id, observation, offset) => unwrap(await ctx.remote.researchWorkspace.observationChoices(id,
      observation, offset)),
    observationReviews: async (id, observation, offset) => unwrap(await ctx.remote.researchWorkspace.observationReviews(id,
      observation, offset)),
  ...
  const injectProps = (): WorkspaceInjected => ({
  ...
  const injectObservations = (): ObservationsInjected => ({
  ...
  const injectTasks = (): TasksInjected => ({
