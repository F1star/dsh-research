...
import { defineTool, type InferValue, type JsonValue, type ToolRunContext } from '@deepseek-ai/dsh-tools'
import type {} from '@deepseek-ai/dsh-system-prompt'
import { ResearchAuthorId, ResearchQuestionId } from '../research-information/index.ts'
import { ResearchPaperId, ResearchSourceVersionId } from '../research-library/index.ts'
import type {} from '../research-task/index.ts'
import type { ResearchTaskId, ResearchTaskRecord, ResearchTaskMutationResult, UpdateResearchTaskRequest } from '../research-task/types.ts'
...
const STAGES = ['acquisition', 'extraction', 'review', 'comparison', 'synthesis', 'export'] as const
const ARTIFACTS = ['evidenceIds', 'claimIds', 'claimReviewIds', 'observationIds', 'observationReviewIds', 'comparisonProtocolIds', 'synthesisIds'] as const
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const render = (value: JsonValue) => [{ type: 'text' as const, text: JSON.stringify(value, null, 2) }]
  const meta = (value: JsonValue): JsonValue => ({ kind: 'dsh/research-task', version: 1, value })
  const fits = (value: Output): boolean =>
    Buffer.byteLength(JSON.stringify({ value, content: render(value), presentationMeta: meta(value) })) <= maxOutputBytes
  const bounded = (value: Output): Output => {
    if (!fits(value)) throw new Error('Complete task output exceeds maxOutputBytes; reduce the page size or increase the configured limit.')
    return value
  }
  const page = <T>(values: readonly T[], offset: number, project: (items: readonly T[]) => Output): Output => {
    if (!Number.isSafeInteger(offset) || offset < 0 || offset > values.length) throw new Error('Task offset is outside the selected collection.')
    let count = Math.min(maxItems, values.length - offset)
    for (;;) {
      const result: Output = { ...project(values.slice(offset, offset + count)), total: values.length,
        next_offset: offset + count < values.length ? offset + count : null }
      if (fits(result)) return result
      if (count <= 1) return bounded(result)
      count--
    }
  }
  const output = { schema: OUTPUT, render: (_args: unknown, value: Output) => render(bounded(value)),
    presentationMeta: (_args: unknown, value: Output) => meta(bounded(value)) }
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args): Promise<Output> {
      return Promise.resolve().then(() => {
        const view = ctx.researchTasks.get(args.task_id as ResearchTaskId)
        if (view === undefined) return bounded({ status: 'task-not-found' })
        const task = view.task
        const base = { status: 'ready', task: summary(task) }
        const offset = args.offset ?? 0
        const selectedView = args.view ?? 'progress'
        if (selectedView === 'progress') return page(view.issues, offset, rows => ({ ...base, issues: [...rows],
        ...
        if (selectedView === 'history') return page(task.checkpoints, offset, rows => ({ ...base, history: rows.map(checkpoint => ({
        ...
        if (args.checkpoint_revision !== undefined && checkpoint === undefined) throw new Error('Task checkpoint revision was not found.')
        if (selectedView === 'sources') return page(checkpoint?.sources ?? task.sources, offset, rows => ({ ...base,
          sources: rows.map(source => ({ paper_id: source.paperId, source_version_id: source.sourceVersionId })) }))
        if (checkpoint === undefined || args.artifact_kind === undefined) throw new Error('Artifacts require checkpoint_revision and artifact_kind.')
        return page<string>(checkpoint.artifacts[args.artifact_kind], offset, rows => ({ ...base, artifact_ids: [...rows] }))
      })
    },
  }))
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec): Promise<Output> {
      ...
      let result: ResearchTaskMutationResult
      if (args.action === 'create') {
        if (args.question_id === undefined || args.kind === undefined) throw new Error('Task creation requires question_id and kind.')
        result = await ctx.researchTasks.create({ questionId: ResearchQuestionId(args.question_id), kind: args.kind, author })
      } else {
        if (args.task_id === undefined || args.revision === undefined) throw new Error('Task updates require task_id and revision.')
        const common = { taskId: args.task_id as ResearchTaskId, expectedRevision: args.revision, author }
        let request: UpdateResearchTaskRequest
        if (args.action === 'checkpoint') {
          ...
          if (args.stage === undefined || args.reason === undefined) throw new Error('Rewind requires stage and reason.')
          request = { ...common, action: 'rewind', stage: args.stage, reason: args.reason }
        } else {
          if (args.reason === undefined) throw new Error('Task phase changes require a reason.')
          request = { ...common, action: args.action, reason: args.reason }
        }
        result = await ctx.researchTasks.update(request)
      }
      if (result.status === 'saved') return bounded({ status: 'saved', task_id: result.view.task.id, revision: result.view.task.revision })
      if (result.status === 'stale-revision' || result.status === 'stale-question') return bounded({ status: result.status, current_revision: result.currentRevision })
      if (result.status === 'cannot-advance') return bounded({ status: result.status, issues: [...result.issues] })
      return bounded({ status: result.status })
    },
  }))
}

function summary(task: ResearchTaskRecord): InferValue<typeof TASK> {
...
function agentAuthor(exec: ToolRunContext) {
  if (exec.agent === undefined) throw new Error('Research task writes require an owning agent session.')
