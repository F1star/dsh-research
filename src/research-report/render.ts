...
import { observationReport } from './observations.ts'
import { researchObservationState } from '../research-information/index.ts'
import type { ResearchQuestionRecord, ResearchClaim, ResearchClaimReview, ResearchFacet } from '../research-information/types.ts'
import type { ResearchPaperRecord } from '../research-library/types.ts'
import type { ResearchReportBundle, ResearchReportDigest, ResearchReportFile } from './types.ts'

interface Paragraph { readonly text: string; readonly citations: readonly string[] }
interface Section { readonly title: string; readonly paragraphs: Paragraph[] }

const facets: Record<ResearchFacet, string> = {
  aim: '研究目标', method: '方法', dataset: '数据集', metric: '指标', result: '结果',
  limitation: '局限', 'validity-threat': '有效性威胁', other: '其他',
}
const decisions = { accepted: '人工接受', revised: '修改后接受', rejected: '人工拒绝' } as const
const support = { supports: '支持', partial: '部分支持', unsupported: '不支持', uncertain: '尚不能确定' } as const
const relations = { supports: '支持', contradicts: '反驳', qualifies: '限定', background: '背景' } as const
const stances = { agreement: '一致', conflict: '冲突', qualification: '限定', 'open-question': '开放问题' } as const
 ...
 */
export function renderReport(question: ResearchQuestionRecord, papers: readonly ResearchPaperRecord[]): ResearchReportBundle {
  ...
  const reviews = new Map<ResearchClaim['id'], ResearchClaimReview>()
  ...
  const citeClaim = (claim: ResearchClaim): string[] => [...new Set(claim.evidenceLinks.map((link) => {
    ...
    if (source === undefined) throw new Error('Claim references unavailable evidence')
    return citationKey(source.paperId)
  }))]
  const reviewText = (claim: ResearchClaim): string => {
  ...
  const section = (title: string): Section => { const value = { title, paragraphs: [] }; sections.push(value); return value }
  const add = (target: Section, text: string, citations: readonly string[] = []): void => { target.paragraphs.push({ text, citations }) }
  ...
  for (const value of question.syntheses) {
    if (replacedSyntheses.has(value.id)) continue
    for (const finding of value.findings) {
      ...
      const referenced = finding.claimIds.map((id) => {
        ...
        if (claim === undefined) throw new Error('Finding references an unavailable current claim')
        return claim
      })
      includedFindings++
      ...
      add(synthesis, `${finding.kind === 'source-summary' ? '来源综合' : '作者推断'} · ${stances[finding.stance]} · ${status}：${finding.text}`,
        [...new Set(referenced.flatMap(citeClaim))])
  ...
  if (includedFindings === 0) add(synthesis, '尚无可纳入正文的当前综合结论。')
  if (excludedFindings === 0) add(omitted, '没有因引用失效或被拒绝而排除的当前综合结论。')
  ...
  for (const claim of currentClaims) {
    ...
    if (claim.evidenceLinks.length === 0) add(target, '未引用证据；不能作为论文原文陈述。')
    for (const link of claim.evidenceLinks) add(target, `证据 ${link.evidenceId}：${relations[link.relation]}。定位和精确文本见证据附录。`)
    if (review !== undefined) {
      add(target, `审阅人：${review.createdBy.id}；时间：${review.createdAt}；理由：${review.rationale}`)
      if (review.qualifications !== undefined) add(target, `限定条件：${review.qualifications}`)
      if (review.counterEvidenceIds.length > 0) add(target, `记录的反证：${review.counterEvidenceIds.join('、')}。完整内容见证据附录。`)
    }
  }
  if (currentClaims.length === 0) add(statements, '尚无论断。')
  if (!currentClaims.some(claim => reviews.get(claim.id)?.decision === 'rejected')) add(rejected, '没有当前被拒绝的论断。')
  const results = section('结构化结果与比较条件')
  for (const paragraph of observationReport(question)) add(results, paragraph.text, paragraph.paperIds.map(citationKey))
  ...
  for (const note of question.readingNotes) {
    if (replacedNotes.has(note.id)) continue
    ...
    if (source === undefined) throw new Error('Note references unavailable evidence')
  ...
  if (notes.paragraphs.length === 0) add(notes, '尚无当前阅读笔记。')
  ...
  for (const paper of papers) {
    ...
    const missing = (Object.keys(facets) as ResearchFacet[]).filter(facet => !recorded.has(facet))
    ...
    if (missingMetadata.length > 0) add(gaps, `书目信息未记录：${missingMetadata.join('、')}；导出不会补写或推测这些字段。`, [citationKey(paper.id)])
  }
  if (papers.length === 0) add(gaps, '尚未捕获论文证据，无法评估论文维度覆盖。')
  ...
  for (const value of question.evidence) {
    ...
    if (value.selection !== undefined) add(appendix, `精确摘录：${value.selection.text}`)
  ...
  const audit: unknown = JSON.parse(stableStringify({ format: 'research-report-audit', version: 1, question, papers }))
  ...
  return { questionId: question.id, revision: question.revision,
    digest: `sha256:${createHash('sha256').update(JSON.stringify(files.map(file => [file.name, file.mediaType, file.text]))).digest('hex')}` as ResearchReportDigest, files,
...
function citationKey(id: ResearchPaperRecord['id']): string { return `p${id.replaceAll('-', '')}` }

function toCsl(paper: ResearchPaperRecord) {
...
function markdown(title: string, sections: readonly Section[], papers: readonly ResearchPaperRecord[]): string {
  ...
  lines.push('## 参考文献索引', '')
  for (const paper of papers) lines.push(`- ${citationKey(paper.id)}：${escapeMarkdown(paper.metadata.title.value)}。`, '')
  return lines.join('\n').trimEnd() + '\n'
}
...
function latex(title: string, sections: readonly Section[]): string {
...
function escapeLatex(text: string): string {
  ...
  return text.replace(/[\\{}$&#%_^~]/gu, character => escapes[character as keyof typeof escapes])
}

function latexParagraph(text: string): string {
