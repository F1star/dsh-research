...
import type { ResearchQuestionRecord, ResearchObservation, ResearchObservationReportedContext } from '../research-information/types.ts'
 ...
 */
export function observationReport(question: ResearchQuestionRecord): ObservationReportParagraph[] {
  const paragraphs: ObservationReportParagraph[] = []
  ...
  const add = (text: string, paperIds: ObservationReportParagraph['paperIds'] = []): void => { paragraphs.push({ text, paperIds }) }
  for (const observation of question.observations) {
    ...
    if (state.stale) add('该结果的来源或规范化实体已被替换，需要重新核对。')
    if (state.rejectedClaimIds.length > 0) add(`该结果引用了已拒绝的来源表述：${state.rejectedClaimIds.join('、')}。历史人工接受不解除此警告。`)
    if (review !== null) {
      ...
      if (review.qualifications !== undefined) add(`结果限定条件：${review.qualifications}`)
      if (review.counterEvidenceIds.length > 0) add(`结果反证：${review.counterEvidenceIds.join('、')}。完整内容见证据附录。`)
    }
  }
  if (paragraphs.length === 0) add('尚无当前结构化结果。')
  ...
  for (const protocol of question.comparisonProtocols) {
    if (retiredProtocols.has(protocol.id)) continue
...
function statusText(status: 'not-recorded' | 'not-applicable'): string { return status === 'not-recorded' ? '尚未记录' : '不适用' }
function contextText(value: ResearchObservationReportedContext): string {
  return value.status === 'reported' ? `${value.value}（来源表述 ${value.sourceClaimId}）` : statusText(value.status)
}
function uncertaintyText(value: ResearchObservation['uncertainty']): string {
  if (value.status !== 'reported') return statusText(value.status)
