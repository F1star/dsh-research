...
import { RUN_CODE_NAME, type ToolExecution } from '@deepseek-ai/dsh-tools'
 ...
 */
export function taskToolDenial(exec: ToolExecution, view: ResearchTaskView, documents: readonly ResearchDocumentId[]): string | undefined {
  if (exec.name === RUN_CODE_NAME) return undefined
  const args = exec.arguments
  if (args === null || typeof args !== 'object' || Array.isArray(args)) return 'Research task tools require object arguments.'
  if (questionTools.has(exec.name)) {
    if (!('question_id' in args) || args.question_id !== view.task.questionId) return 'Use the research question assigned to this task.'
    if (exec.name === 'research_evidence_capture'
      && (!('document_id' in args) || !documents.some(id => id === args.document_id))) return 'Capture only the selected exact sources.'
    return undefined
  }
  ...
  if (exec.name === 'research_task_get' || exec.name === 'research_task_write') {
    if (!('task_id' in args) || args.task_id !== view.task.id) return 'Use the assigned research task.'
    if (exec.name === 'research_task_get') return undefined
    if (!('action' in args) || args.action !== 'checkpoint') return 'Task execution may record checkpoints; researchers control source selection and recovery.'
    if ('stage' in args && args.stage === 'acquisition') return 'Researchers select the exact task sources before execution.'
    return undefined
  }
 ...
 */
export function taskRunPrompt(view: ResearchTaskView): string {
