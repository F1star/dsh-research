...
import { researchTaskDomainSpec, researchTaskSchema, createResearchTaskSchema, updateResearchTaskSchema } from "./spec.js";
import { inspectStage, taskStages } from "./stages.js";
export { researchTaskDomainSpec } from "./spec.js";
...
export class ResearchTasks extends Service {
    ...
    async [Service.init]() {
        ...
        if (this.tasks.size > this.limits.maxTasks)
            throw new Error('Stored tasks exceed maxTasks');
        for (const [id, task] of this.tasks.entries()) {
            if (id !== task.id)
                throw new Error('Task key differs from its saved id');
            this.validate(task);
        }
    }
    /**
    ...
    create(request) {
        ...
        return this.enqueue(async () => {
            if (this.ctx.researchInformation.get(input.questionId) === undefined)
                return { status: 'question-not-found' };
            if (this.table().size >= this.limits.maxTasks)
                throw new Error('Research tasks exceed maxTasks');
            const time = new Date().toISOString();
            const task = { id: randomUUID(), questionId: input.questionId,
    ...
    update(request) {
        ...
        return this.enqueue(async () => {
            ...
            if (task.revision !== input.expectedRevision)
                return { status: 'stale-revision', currentRevision: task.revision };
            const view = this.view(task);
            const deny = (...issues) => ({ status: 'cannot-advance', issues, view });
            ...
            switch (input.action) {
                ...
                case 'block': {
                    if (task.phase === 'complete')
                        return deny('Resume or rewind a completed task before changing its phase.');
                    return this.commit({ ...updated, phase: input.action === 'pause' ? 'paused' : 'blocked', reason: input.reason }, false);
                }
                case 'resume': {
                    if (task.phase === 'active' && !view.requiresResume && !view.stale)
                        return deny('The task is already active.');
                    if (view.nextStage === null)
                        return deny('The completed task still matches its scientific inputs; rewind explicitly to do new work.');
                ...
                case 'rewind': {
                    ...
                    if (index < 0 || index > view.completedStages.length)
                        return deny('Rewind may select only a completed or current workflow stage.');
                ...
                case 'checkpoint': {
                    if (task.phase !== 'active' || view.requiresResume)
                        return deny('Explicitly resume this task before recording a checkpoint.');
                    if (input.expectedQuestionRevision !== view.questionRevision)
                        return { status: 'stale-question', currentRevision: view.questionRevision };
                    if (input.stage !== view.nextStage)
                        return deny(`The next stage is ${view.nextStage ?? 'none'}.`);
                    if (input.stage !== 'acquisition' && input.sources !== undefined)
                        return deny('Source selection belongs to acquisition; rewind to change it.');
                    if ((input.stage === 'comparison') !== (input.comparisonOutcome !== undefined))
                        return deny('Only comparison checkpoints require a comparison outcome.');
                    const sources = input.stage === 'acquisition' ? input.sources ?? [] : task.sources;
                    const question = this.question(task);
                    const inspection = inspectStage(input.stage, task.kind, question, sources, this.ctx.researchLibrary.list(), () => this.ctx.researchReport.render({ questionId: question.id, expectedRevision: question.revision }), input.comparisonOutcome);
                    if (inspection.issues.length > 0)
                        return deny(...inspection.issues);
                    const checkpoint = { taskRevision: updated.revision, stage: input.stage,
    ...
    view(task) {
        ...
        for (const revision of task.checkpointRevisions) {
            ...
            const inspection = inspectStage(checkpoint.stage, task.kind, question, task.sources, this.ctx.researchLibrary.list(), () => this.ctx.researchReport.render({ questionId: question.id, expectedRevision: question.revision }), checkpoint.comparisonOutcome);
            issues = inspection.issues;
            if (issues.length === 0 && inspection.basisDigest !== checkpoint.basisDigest)
                issues = [`The ${checkpoint.stage} inputs changed; reassess that stage.`];
            if (issues.length > 0)
                break;
        ...
        if (!stale && nextStage !== null) {
            issues = inspectStage(nextStage, task.kind, question, task.sources, this.ctx.researchLibrary.list(), () => this.ctx.researchReport.render({ questionId: question.id, expectedRevision: question.revision })).issues;
        }
    ...
    question(task) {
        ...
        if (question === undefined)
            throw new Error('Research task references an unavailable question');
        return question;
    }
    validate(task) {
        ...
        const question = this.question(task);
        const stages = taskStages(task.kind);
        if (task.sources.length > this.limits.maxSourcesPerTask)
            throw new Error('Task sources exceed maxSourcesPerTask');
        if (task.checkpoints.length > this.limits.maxCheckpointsPerTask)
            throw new Error('Task history exceeds maxCheckpointsPerTask');
        if (Buffer.byteLength(JSON.stringify(task)) > this.limits.maxTaskBytes)
            throw new Error('Task record exceeds maxTaskBytes');
        ...
        if (texts.some(value => Array.from(value).length > this.limits.maxTextChars))
            throw new Error('Task text exceeds maxTextChars');
        const updatedTime = Date.parse(task.updatedAt);
        if (updatedTime < Date.parse(task.createdAt))
            throw new Error('Task update precedes creation');
        ...
        for (const checkpoint of task.checkpoints) {
            ...
            if (checkpoint.sources.length > this.limits.maxSourcesPerTask)
                throw new Error('Checkpoint sources exceed maxSourcesPerTask');
            ...
            const sourceIssues = inspectStage('acquisition', task.kind, question, checkpoint.sources, this.ctx.researchLibrary.list(), () => { throw new Error('Acquisition does not render reports'); }).issues;
            if (sourceIssues.length > 0)
                throw new Error(`Saved checkpoint sources are invalid: ${sourceIssues.join(' ')}`);
            for (const key of Object.keys(ids)) {
                const allowed = ids[key];
                if (checkpoint.artifacts[key].some(id => !allowed.has(id)))
                    throw new Error(`Task checkpoint references unavailable ${key}`);
            }
            previousRevision = checkpoint.taskRevision;
            previousTime = checkpointTime;
        }
        if (task.checkpointRevisions.length > stages.length)
            throw new Error('Task has more checkpoints than workflow stages');
        ...
        if ((task.phase === 'complete') !== (task.checkpointRevisions.length === stages.length))
            throw new Error('Task phase disagrees with its saved progress');
    }
    table() {
        if (this.tasks === undefined)
            throw new Error('Research task storage is unavailable');
        return this.tasks;
    }
    assertOpen() {
        if (this.closed)
    ...
    async commit(task, activate) {
        this.validate(task);
        await this.table().put(task.id, task);
        if (activate)
            this.activated.add(task.id);
        else
            this.activated.delete(task.id);
