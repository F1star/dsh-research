...
export class ResearchInformation extends Service {
    ...
    validateStoredRecord(record) {
        ...
        for (const protocol of record.comparisonProtocols) {
            ...
            for (const observationId of protocol.observationIds) {
                ...
                this.assertObservationActiveAtCreation(protocol.id, observation.id, protocolTimestamp, supersedingObservationsById.get(observation.id));
                this.assertObservationFreshAtCreation(record, protocol, observation, protocolTimestamp, supersedingClaimsById, supersedingEntitiesById);
                observations.push(observation);
    ...
    assertObservationActiveAtCreation(protocolId, observationId, protocolTimestamp, successor) {
        if (successor !== undefined && Date.parse(successor.createdAt) < protocolTimestamp) {
            throw inconsistent(`comparison protocol '${protocolId}' references observation '${observationId}' superseded before protocol creation`);
        }
    }
    assertObservationFreshAtCreation(record, protocol, observation, protocolTimestamp, supersedingClaimsById, supersedingEntitiesById) {
        const claimIds = observationClaimIds(observation);
        for (const claimId of claimIds) {
            this.assertClaimActiveAtCreation('comparison protocol', protocol.id, claimId, protocolTimestamp, supersedingClaimsById.get(claimId));
        }
        for (const entityId of observationEntityIds(observation)) {
            const successor = supersedingEntitiesById.get(entityId);
            if (successor !== undefined && Date.parse(successor.createdAt) < protocolTimestamp) {
                throw inconsistent(`comparison protocol '${protocol.id}' references observation '${observation.id}' with a superseded entity`);
            }
