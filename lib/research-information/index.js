...
export class ResearchInformation extends Service {
    ...
    constructor(ctx, config = {}) {
        ...
        this.config = {
            ...
            maxClaimReviewsPerQuestion: positiveSafeInteger('maxClaimReviewsPerQuestion', config.maxClaimReviewsPerQuestion ?? DEFAULT_MAX_CLAIM_REVIEWS_PER_QUESTION),
            ...
            maxObservationReviewsPerQuestion: positiveSafeInteger('maxObservationReviewsPerQuestion', config.maxObservationReviewsPerQuestion ?? DEFAULT_MAX_OBSERVATION_REVIEWS_PER_QUESTION),
            maxCounterEvidencePerObservationReview: positiveSafeInteger('maxCounterEvidencePerObservationReview', config.maxCounterEvidencePerObservationReview ?? DEFAULT_MAX_COUNTER_EVIDENCE_PER_OBSERVATION_REVIEW),
    ...
    async writeClaimNow(request) {
        ...
        if (result.status === 'created')
            await this.requireTable().put(result.question.id, result.question);
        return result;
    }
    ...
    async reviewClaimNow(request) {
        if (request.author.kind !== 'researcher')
        ...
        if (claim === undefined)
            return { status: 'claim-not-found', claimId: request.claimId };
        if (!isActiveClaim(current, claim.id))
            return { status: 'claim-inactive', claimId: claim.id };
        ...
        const review = {
            ...decision, id: randomUUID(), claimId: claim.id,
    ...
    async reviewObservationNow(request) {
        if (request.author.kind !== 'researcher')
            return { status: 'researcher-required' };
        const table = this.requireTable();
        const current = table.get(request.questionId);
        ...
        if (observation === undefined)
            return { status: 'observation-not-found', observationId: request.observationId };
        if (!isActiveObservation(current, observation.id))
            return { status: 'observation-inactive', observationId: observation.id };
        ...
        if (request.decision === 'revised') {
            ...
            if (prepared.status !== 'created')
                return prepared;
            candidate = prepared.question;
            reviewed = candidate.observations[candidate.observations.length - 1];
        ...
        if (request.decision !== 'rejected') {
            if (isObservationStale(candidate, reviewed))
                return { status: 'observation-stale', observationId: observation.id };
            const rejected = observationRejectedClaim(candidate, reviewed);
            if (rejected !== undefined)
                return { status: 'observation-rejected-claim', claimId: rejected };
        }
        const now = candidate === current ? mutationTimestamp(current) : candidate.updatedAt;
        const review = { ...decision, id: randomUUID(),
    ...
    async writeComparisonProtocolNow(request) {
        ...
        for (const observationId of request.observationIds) {
            ...
            if (rejectedClaim !== undefined)
                return { status: 'observation-rejected-claim', claimId: rejectedClaim };
    ...
    normalizeReviewAssessment(request) {
        return {
            expectedRevision: nonNegativeSafeInteger('expectedRevision', request.expectedRevision),
    ...
    validateReviewHistory(record, reviews, kind, counterEvidenceLimit) {
        ...
        for (const review of reviews) {
            ...
            if (review.qualifications !== undefined)
                this.assertCanonicalAuthoredText('review qualifications', review.qualifications);
            this.assertCanonicalAuthor(review.createdBy);
            if (review.createdBy.kind !== 'researcher')
                throw inconsistent(`${kind} review requires a researcher`);
            ...
            if (review.questionRevision <= previousRevision || review.questionRevision > record.revision
                ...
                throw inconsistent(`${kind} review history is not in revision order`);
    ...
    validateStoredObservationReviews(record) {
        ...
        for (const review of revisions) {
            ...
            if (timestamp < previousTime)
                throw inconsistent('researcher decision timestamps disagree with committed revisions');
            previousTime = timestamp;
        }
        for (const review of record.observationReviews) {
            ...
            if (review.decision !== 'rejected') {
                const reviewed = review.decision === 'revised' ? successor : observation;
 ...
 */
export function researchObservationState(record, observation) {
...
function getResearchObservationReview(record, id) {
...
function rejectedObservationClaims(record, observation) {
    return [...new Set(observationClaimIds(observation))].filter(id => record.claimReviews.findLast(review => review.claimId === id || (review.decision === 'revised' && review.replacementClaimId === id))?.decision === 'rejected');
}
function observationRejectedClaim(record, observation) {
