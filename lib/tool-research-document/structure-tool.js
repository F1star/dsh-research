...
import { EXTRACTION_SCHEMA, LOCATOR_SCHEMA, boundedOptionalCount, nonNegativeOptionalCount, parseBlockId, parseDocumentId, projectLocator, taggedMeta, } from "./navigation.js";
const kind = { type: 'string', required: true, enum: ['table', 'formula', 'figure'] };
const field = { type: 'string', enum: ['text', 'latex', 'description', 'classification', 'caption', 'footnote', 'cell'] };
const common = {
 ...
 */
export function registerStructureTool(ctx, config) {
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (args.block_id !== undefined && args.kind !== undefined)
                throw new Error('kind filters the object list only.');
            ...
            const base = {
                ...
                extraction: document.extraction, review_status: 'unreviewed',
            };
            ...
            if (block === undefined)
                throw new Error('block_id does not identify a scientific object in this parser revision.');
...
function selectText(block, field, index) {
    ...
    switch (field) {
        ...
        case 'latex':
            result = structure.kind === 'formula' ? structure.latex : undefined;
            break;
        case 'description':
            result = structure.kind === 'figure' ? structure.description : undefined;
            break;
        case 'classification':
            result = structure.kind === 'figure' ? structure.classification : undefined;
            break;
        case 'caption':
            result = structure.kind === 'formula' ? undefined : structure.captions[position];
            break;
        case 'footnote':
            result = structure.kind === 'formula' ? undefined : structure.footnotes[position];
            break;
        case 'cell':
            result = tableOf(structure)?.cells[position]?.text;
            break;
        default: return assertNever(field);
    }
    if (result === undefined)
        throw new Error('The selected field or field_index does not exist on this scientific object.');
    return result;
}
...
function checkOffset(offset, total) {
    if (offset > total)
        throw new Error(`Offset exceeds the available item or character count (${total}).`);
}
function render(value) {
    return [{ type: 'text', text: `Unreviewed scientific extraction. Formula notation and chart data are machine interpretations, not block quotations.\n${JSON.stringify(value)}` }];
}
function fitItems(count, make, config, reduceCellText = false) {
    let textLimit = config.maxStructureTextChars;
    for (;;) {
        const fits = (value) => Buffer.byteLength(JSON.stringify({
...
//# sourceMappingURL=structure-tool.js.map