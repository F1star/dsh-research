...
const strings = { type: 'array', required: true, items: { type: 'string' } };
const nullableText = { oneOf: [{ type: 'string' }, { type: 'null' }], required: true };
const status = { type: 'string', required: true, enum: ['extracted', 'not-requested', 'unavailable'] };
 ...
 */
export function projectStructure(value) {
    switch (value.kind) {
        case 'table':
