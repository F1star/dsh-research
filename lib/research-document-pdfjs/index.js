...
export class PdfJsResearchDocumentParser {
    ctx;
    id = PDFJS_PARSER_ID;
    config;
    constructor(ctx, config = {}) {
        ...
        this.config = {
            ...
            maxTextItemsPerPage: positiveSafeInteger('maxTextItemsPerPage', config.maxTextItemsPerPage ?? DEFAULT_MAX_TEXT_ITEMS_PER_PAGE),
        };
    }
    ...
    async parse(request, signal) {
        ...
        try {
            ...
            if (document.numPages > this.config.maxPages) {
                throw new ResearchDocumentError(`PDF has ${document.numPages} pages; the configured limit is ${this.config.maxPages}`, 'RESEARCH_DOCUMENT_PDF_PAGE_LIMIT');
            }
            const labels = await document.getPageLabels();
            const metadata = await document.getMetadata();
            throwIfAborted(signal);
            const titleValue = metadata.info.Title;
            ...
            for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
                ...
                try {
                    ...
                    if (textItems.length > this.config.maxTextItemsPerPage) {
                        throw new ResearchDocumentError(`PDF page ${pageNumber} has ${textItems.length} text items; the configured limit is ${this.config.maxTextItemsPerPage}`, 'RESEARCH_DOCUMENT_PDF_TEXT_ITEM_LIMIT');
                    }
            ...
            try {
                ...
                if (primaryFailure === undefined) {
                    throw new ResearchDocumentError('PDF.js resource cleanup failed', 'RESEARCH_DOCUMENT_PDF_CLEANUP_FAILED', { cause: error });
                }
...
function lineBlocks(items, viewportTransform, pageWidth, pageHeight) {
...
function positionItem(item, viewportTransform) {
    const transform = multiplyTransforms(requirePdfMatrix('viewport', viewportTransform), requirePdfMatrix('text item', item.transform));
...
function requirePdfMatrix(name, value) {
    if (!Array.isArray(value))
        throw new TypeError(`PDF.js returned a non-array ${name} transform`);
    if (value.length !== 6)
        throw new TypeError(`PDF.js returned an invalid-length ${name} transform`);
    for (const entry of value) {
        if (typeof entry !== 'number')
            throw new TypeError(`PDF.js returned a non-numeric ${name} transform`);
        if (!Number.isFinite(entry))
            throw new TypeError(`PDF.js returned a non-finite ${name} transform`);
    }
    return value;
}
...
function newLine(item, top, right) {
    return {
        text: item.text,
        left: item.x,
        top,
...
function separator(leftText, rightText, gap, height) {
    if (/\s$/u.test(leftText) || /^\s/u.test(rightText))
        return '';
...
function classifyHeading(text, height, bodyHeight) {
    if (bodyHeight <= 0 || text.length > 180 || height < bodyHeight * 1.25)
        return undefined;
...
function normalizedRect(left, top, right, bottom, pageWidth, pageHeight) {
...
function normalizeParseError(error, signal) {
    if (signal?.aborted)
        return abortedError();
    if (error instanceof ResearchDocumentError)
        return error;
    if (error instanceof Error && error.name === 'PasswordException') {
        return new ResearchDocumentError('encrypted PDF requires a password and cannot be imported by this provider', 'RESEARCH_DOCUMENT_PDF_ENCRYPTED', { cause: error });
    }
    return new ResearchDocumentError(`PDF.js could not parse the document: ${errorMessage(error)}`, 'RESEARCH_DOCUMENT_PDF_PARSE_FAILED', { cause: error });
}
