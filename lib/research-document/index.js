...
export { researchDocumentParseResultSchema } from "./schema.js";
/**
...
export class ResearchDocumentRuntime extends Service {
    ...
    registerParser(parser) {
        if (this.parsers.has(parser.id)) {
            throw new ResearchDocumentError(`a research-document parser with id "${parser.id}" is already registered`, 'RESEARCH_DOCUMENT_DUPLICATE_PROVIDER');
        }
    ...
    async import(request, signal) {
        ...
        if (cached !== undefined) {
            if (cached.mediaType.toLowerCase() !== request.mediaType.toLowerCase()) {
                throw new ResearchDocumentError(`research document "${documentId}" is retained as ${cached.mediaType}, not ${request.mediaType}`, 'RESEARCH_DOCUMENT_MEDIA_TYPE_MISMATCH');
            }
     ...
     */
    async restore(documentId, parser) {
        const cached = this.documents.get(documentId);
        ...
        if (snapshot === undefined) {
            throw new ResearchDocumentError(`research document "${documentId}"${parser ? ` at ${parser.id}@${parser.version}` : ''} is not retained or archived; import it again`, 'RESEARCH_DOCUMENT_NOT_FOUND');
        }
    ...
    get(documentId) {
        ...
        if (document === undefined) {
            throw new ResearchDocumentError(`research document "${documentId}" is not retained; import it again`, 'RESEARCH_DOCUMENT_NOT_FOUND');
        }
        this.touch(documentId, document);
        return document;
    }
    ...
    outline(document) {
        return blocksOf(typeof document === 'string' ? this.get(document) : document)
            .filter((block) => block.kind === 'heading' && block.headingLevel !== undefined)
     ...
     */
    structures(document, kind) {
        return blocksOf(typeof document === 'string' ? this.get(document) : document)
            .filter((block) => block.structure !== undefined && (kind === undefined || block.structure.kind === kind));
    }
    /**
     ...
     */
    search(document, query, maxResults) {
     ...
     */
    read(document, blockId, before, after) {
        ...
        if (focus === -1) {
            throw new ResearchDocumentError(`block "${blockId}" does not belong to research document "${snapshot.id}"`, 'RESEARCH_DOCUMENT_BLOCK_NOT_FOUND');
        }
        return {
    ...
    resolveParser(mediaType) {
        const configuredId = this.config.parserProvider;
        if (configuredId !== undefined) {
            const configured = this.parsers.get(configuredId);
            if (configured === undefined) {
                throw new ResearchDocumentError(`configured research-document parser "${configuredId}" is not registered`, 'RESEARCH_DOCUMENT_PROVIDER_CONFIGURED_MISSING');
            }
            if (!configured.available() || !configured.supports(mediaType)) {
                throw new ResearchDocumentError(`configured research-document parser "${configuredId}" cannot parse ${mediaType}`, 'RESEARCH_DOCUMENT_PROVIDER_CONFIGURED_UNAVAILABLE');
            }
            return configured;
        }
        const usable = [...this.parsers.values()].filter(parser => parser.available() && parser.supports(mediaType));
        if (usable.length === 0) {
            throw new ResearchDocumentError(`no usable research-document parser accepts ${mediaType}`, 'RESEARCH_DOCUMENT_PROVIDER_UNAVAILABLE');
        }
        if (usable.length > 1) {
            throw new ResearchDocumentError(`multiple research-document parsers accept ${mediaType} (${usable.map(parser => parser.id).join(', ')}); configure one explicitly`, 'RESEARCH_DOCUMENT_PROVIDER_AMBIGUOUS');
        }
        return usable[0];
    }
    touch(documentId, document) {
...
function materializeDocument(documentId, mediaType, parserId, parsed) {
    let readingOrder = 0;
    const sectionPath = [];
    const pages = parsed.pages.map(page => ({
        ...
        blocks: page.blocks.map((block, blockIndex) => {
            ...
            const id = ResearchDocumentBlockId(`block:${hashText(`${documentId}\0${parserId}\0${parsed.parserVersion}\0${page.pageIndex}\0${blockIndex}`)}`);
...
function searchScore(text, phrase, terms) {
