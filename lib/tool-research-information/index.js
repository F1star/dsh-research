...
export function apply(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Append immutable structured synthesis findings over active claims. Source summaries require evidence-backed source-statement claims; inference findings remain visibly labelled.',
        parameters: {
            question_id: { type: 'string', required: true, description: 'Target research-question UUID.' },
            revision: { type: 'integer', required: true, description: 'Exact current question revision.' },
            findings: {
...
function projectAudit(ctx, record, observationState, budget, offset, maximum) {
    ...
    const uncitedInferenceFindingIds = record.syntheses.flatMap(synthesis => synthesis.findings
        .filter(finding => finding.kind === 'inference' && finding.claimIds.length === 0)
