...
import { ResearchAuthorId, researchObservationState, ResearchClaimId, ResearchComparisonProtocolId, ResearchDecimal, ResearchEntityId, ResearchEvidenceId, ResearchEvidenceTextHash, ResearchObservationId, ResearchQuestionId, ResearchReadingNoteId, ResearchSynthesisId, } from "../research-information/index.js";
import { ResearchPaperId, } from "../research-library/index.js";
import { defineTool } from '@deepseek-ai/dsh-tools';
import { RESEARCH_REVIEW_WARNING_CODES, renderResearchReview, } from "./review-render.js";
...
const ENTITY_KINDS = ['method', 'dataset', 'metric'];
const METHOD_ROLES = ['proposed', 'baseline', 'other'];
...
export function apply(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: outputDefinition('Research question', 'dsh/research-question-get', GET_OUTPUT_SCHEMA, resolved, value => formatResearchQuestionGet(value, resolved.maxOutputTextChars)),
        isConcurrencySafe: () => true,
        execute(args) {
            ...
            const decimalTextOffset = nonNegativeSafeInteger('decimal_text_offset', args.decimal_text_offset ?? 0);
            ...
            if (view === 'notes') {
                ...
                const projectedNotes = page.values.map(note => projectReadingNote(note, activeNotes.has(note.id), question, budget, textOffset, resolved.maxOutputTextChars));
            ...
            if (view === 'entities') {
                ...
                const values = question.entities.filter(entity => (args.entity_kind === undefined || entity.kind === args.entity_kind)
                    && (entityQuery === undefined || comparableText(entity.canonicalName).includes(entityQuery)));
                const page = pageItems(values, offset, maxItems);
                const entities = page.values.map(entity => projectEntity(entity, activeEntities.has(entity.id), activeClaims, question, budget));
            ...
            if (view === 'observations') {
                ...
                const values = question.observations.filter(observation => (observationId === undefined || observation.id === observationId)
                ...
                const decimalPage = args.decimal_field === undefined || page.values[0] === undefined
                    ? undefined
                    : budget.pagedText(observationDecimalValue(page.values[0], args.decimal_field), decimalTextOffset, decimalTextRenderCapacity(observationDecimalValue(page.values[0], args.decimal_field), decimalTextOffset, resolved.maxOutputTextChars, page.values[0].id, args.decimal_field));
                const observations = page.values.map(observation => projectObservation(observation, state, budget));
            ...
            if (view === 'comparisons') {
                ...
                const values = question.comparisonProtocols.filter(protocol => comparisonProtocolId === undefined || protocol.id === comparisonProtocolId);
                const page = pageItems(values, offset, maxItems);
                const comparisonProtocols = page.values.map(protocol => projectComparisonProtocol(protocol, activeProtocols.has(protocol.id), state, budget));
                const summary = projectQuestionSummary(question, budget, state);
                const itemPage = projectItemPage(page);
                const referencePage = budget.referencePage();
                return Promise.resolve({
            ...
            const items = [
                ...question.evidence.map(value => ({ kind: 'evidence', value })),
                ...question.claims.map(value => ({ kind: 'claim', value })),
                ...question.syntheses.map(value => ({ kind: 'synthesis', value })),
            ];
            ...
            const syntheses = synthesisValues.map(value => projectSynthesis(value, activeSyntheses.has(value.id), budget, findingOffset));
            const summary = projectQuestionSummary(question, budget);
            const itemPage = projectItemPage(page);
            const referencePage = budget.referencePage();
            return Promise.resolve({
    ...
    ctx.tools.register(defineTool({
        ...
        output: outputDefinition('Research review render', 'dsh/research-review-render', REVIEW_RENDER_OUTPUT_SCHEMA, resolved, value => formatReviewRender(value)),
        isConcurrencySafe: () => true,
        execute(args) {
            const questionId = parseQuestionId(args.question_id);
            ...
            const rendered = renderResearchReview(ctx.researchInformation.get(questionId), synthesisId, {
            ...
            const capacity = rendered.status === 'not-ready'
                ...
                : reviewTextRenderCapacity(markdown, textOffset, resolved.maxOutputTextChars, rendered.status, renderDigest);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const observed = source.observations.some(value => value.parserId === document.parser.id && value.parserVersion === document.parser.version);
            if (!observed) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: outputDefinition('Research comparison protocol write', 'dsh/research-comparison-protocol-write', MUTATION_OUTPUT_SCHEMA, resolved),
        async execute(args, exec) {
...
function outputDefinition(label, kind, schema, config, render = value => formatJson(label, value, config.maxOutputTextChars)) {
    return {
        schema,
        render: (_args, value) => [{
                type: 'text',
        ...
        presentationMeta: (_args, value) => taggedMeta(kind, value),
    };
}
function projectQuestionWrite(result, config) {
...
function projectEvidenceCapture(result, documentId, blockId, config) {
    ...
    return {
        ...projectMutationFailure(result, config),
        document_id: documentId,
        block_id: blockId,
    };
}
function projectClaimWrite(result, config) {
...
function projectEntityWrite(result, config) {
    if (result.status === 'created') {
        const budget = new ProjectionBudget(config);
        const question = projectQuestionSummary(result.question, budget);
        return {
...
function projectObservationWrite(result, config) {
    if (result.status === 'created') {
        const budget = new ProjectionBudget(config);
        const question = projectQuestionSummary(result.question, budget);
        return {
...
function projectComparisonProtocolWrite(result, config) {
    if (result.status === 'created') {
        const budget = new ProjectionBudget(config);
        const question = projectQuestionSummary(result.question, budget);
        return {
...
function projectReadingNoteWrite(result, config) {
    if (result.status === 'created') {
        const budget = new ProjectionBudget(config);
        const question = projectQuestionSummary(result.question, budget);
        return {
...
function projectSynthesisWrite(result, config) {
    if (result.status === 'created') {
        const budget = new ProjectionBudget(config);
        const question = projectQuestionSummary(result.question, budget);
        return {
...
function projectMutationFailure(result, config) {
...
function runtimeCaptureFailure(status, questionId, documentId, blockId) {
    return {
        status,
        question_id: questionId,
        document_id: documentId,
...
function projectQuestionSummary(record, budget, observationState = observationProjectionState(record)) {
    ...
    return {
        ...
        stale_active_entities: record.entities.filter(entity => activeEntities.has(entity.id)
        ...
        stale_active_observations: record.observations.filter(observation => activeObservations.has(observation.id) && staleObservationIds.has(observation.id)).length,
        total_comparison_protocols: record.comparisonProtocols.length,
        active_comparison_protocols: activeComparisonProtocols.size,
        stale_active_comparison_protocols: record.comparisonProtocols.filter(protocol => activeComparisonProtocols.has(protocol.id)
...
function projectEvidence(evidence, budget) {
...
function projectReadingNote(note, active, record, budget, textOffset, renderMaximum) {
    ...
    const text = budget.pagedText(note.text, textOffset, noteTextRenderCapacity(note.text, textOffset, renderMaximum, note.id, evidence.id));
    const pageLabel = evidence.locator.pageLabel === undefined
        ? undefined
        : budget.text(evidence.locator.pageLabel);
    const sectionPath = budget.references(evidence.sectionPath);
...
function projectClaim(claim, active, question, budget) {
...
function projectReviewAssessment(review, budget) {
...
function projectClaimReview(review, budget) {
...
function projectObservationReview(review, budget) {
...
function projectEntity(entity, active, activeClaims, record, budget) {
    ...
    const evidenceIds = uniqueSorted(sourceClaims.flatMap(claim => claim.evidenceLinks.map(link => link.evidenceId)));
...
function observationProjectionState(record) {
    const assessments = new Map(record.observations.map(observation => [observation.id, researchObservationState(record, observation)]));
    const reviewBlockedObservations = new Set([...assessments].filter(([, value]) => value.review?.decision === 'rejected' || value.rejectedClaimIds.length > 0).map(([id]) => id));
    ...
    const entityById = new Map(record.entities.map(entity => [entity.id, entity]));
    ...
    const observationById = new Map(record.observations.map(observation => [observation.id, observation]));
    const paperByObservation = new Map();
    const staleReferenceIndexByObservation = new Map();
    const staleObservationIds = new Set();
    for (const observation of record.observations) {
        paperByObservation.set(observation.id, observationPaperId(observation, claimById, evidenceById));
        const referenceIndex = observationStaleReferenceIndex(observation, activeClaims, activeEntities, entityById, staleSourceClaimIdsByEntity);
        staleReferenceIndexByObservation.set(observation.id, referenceIndex);
        if (referenceIndex.count > 0)
            staleObservationIds.add(observation.id);
    }
    const candidateBuckets = new Map();
    for (const observation of record.observations) {
        if (!activeObservations.has(observation.id) || staleObservationIds.has(observation.id)
            || reviewBlockedObservations.has(observation.id))
            continue;
        if (observationIntrinsicBlockers(observation).length > 0)
            continue;
        ...
        const paperId = projectionIndexValue(paperByObservation, observation.id, 'observation paper');
        const positions = bucket.positionsByPaper.get(paperId);
        if (positions === undefined)
            bucket.positionsByPaper.set(paperId, [position]);
        else
    ...
    for (const observation of record.observations) {
        const blockers = [];
        if (!activeObservations.has(observation.id))
            blockers.push('inactive');
        if (staleObservationIds.has(observation.id))
            blockers.push('stale-reference');
        const assessment = projectionIndexValue(assessments, observation.id, 'observation assessment');
        if (assessment.review?.decision === 'rejected')
            blockers.push('researcher-rejected');
        if (assessment.rejectedClaimIds.length > 0)
            blockers.push('rejected-source-claim');
        blockers.push(...observationIntrinsicBlockers(observation));
        const bucket = blockers.length === 0
            ? projectionIndexValue(candidateBuckets, observationAlignmentSignature(observation), 'observation candidate bucket')
            : undefined;
        const candidateCount = bucket === undefined
            ? 0
            : bucket.observationIds.length - projectionIndexValue(bucket.positionsByPaper, projectionIndexValue(paperByObservation, observation.id, 'observation paper'), 'candidate paper positions').length;
        if (blockers.length === 0 && candidateCount === 0) {
            blockers.push('no-cross-paper-structural-match');
        }
        if (bucket !== undefined)
            candidateBucketByObservation.set(observation.id, bucket);
        candidateCountByObservation.set(observation.id, candidateCount);
        blockersByObservation.set(observation.id, blockers);
    }
    const staleComparisonProtocolIds = new Set();
    const protocolsByObservation = new Map();
    for (const observation of record.observations)
        protocolsByObservation.set(observation.id, []);
    for (const protocol of record.comparisonProtocols) {
        const stale = protocol.observationIds.some(observationId => !activeObservations.has(observationId) || staleObservationIds.has(observationId));
        if (stale)
            staleComparisonProtocolIds.add(protocol.id);
        if (!activeComparisonProtocols.has(protocol.id) || stale
            || protocol.observationIds.some(id => reviewBlockedObservations.has(id)))
            continue;
...
function projectObservation(observation, state, budget) {
    ...
    const candidateCount = projectionIndexValue(state.candidateCountByObservation, observation.id, 'observation candidate count');
    const protocolIds = projectionIndexValue(state.protocolsByObservation, observation.id, 'observation protocols');
    const blockers = projectionIndexValue(state.blockersByObservation, observation.id, 'observation blockers');
    const candidates = budget.indexedReferences(candidateCount, index => crossPaperCandidateAt(projectionIndexValue(state.candidateBucketByObservation, observation.id, 'observation candidate bucket'), paperId, index));
...
function projectReportedContext(context, budget) {
...
function projectObservationUnit(unit, budget) {
...
function projectObservationUncertainty(uncertainty, budget) {
...
function projectObservationCondition(condition, budget) {
...
function projectComparisonProtocol(protocol, active, state, budget) {
    ...
    const staleObservationIds = protocol.observationIds.filter(observationId => !state.activeObservations.has(observationId) || state.staleObservationIds.has(observationId));
...
function observationIntrinsicBlockers(observation) {
    const blockers = [];
    if (observation.unit.status === 'not-recorded')
        blockers.push('unit-not-recorded');
    ...
    if (observation.conditions.status === 'not-recorded')
        blockers.push('conditions-not-recorded');
    return blockers;
}
function observationAlignmentSignature(observation) {
...
function crossPaperCandidateAt(bucket, paperId, candidateIndex) {
    const excludedPositions = projectionIndexValue(bucket.positionsByPaper, paperId, 'candidate paper positions');
    let lower = 0;
    ...
    while (lower < upper) {
        ...
        if (includedThroughMiddle > candidateIndex)
            upper = middle;
        else
            lower = middle + 1;
    }
    return bucket.observationIds[lower];
}
function sortedUpperBound(values, maximum) {
    let lower = 0;
    let upper = values.length;
    while (lower < upper) {
        const middle = Math.floor((lower + upper) / 2);
        if (values[middle] <= maximum)
            lower = middle + 1;
        else
            upper = middle;
    }
...
function observationEvidenceIds(observation, claimById) {
    const claimIds = observationClaimReferences(observation).map(value => value.claimId);
    const claims = claimIds.map(claimId => projectionIndexValue(claimById, claimId, 'observation source claim'));
    return uniqueSorted(claims.flatMap(claim => claim.evidenceLinks.map(link => link.evidenceId)));
}
function observationDecimalValue(observation, field) {
    if (field === 'value')
    ...
    if (field === 'uncertainty-lower')
        return String(value.lower);
    if (field === 'uncertainty-upper')
...
function observationPaperId(observation, claimById, evidenceById) {
    const resultClaim = projectionIndexValue(claimById, observation.resultClaimId, 'observation result claim');
...
function observationStaleReferenceIndex(observation, activeClaims, activeEntities, entityById, staleSourceClaimIdsByEntity) {
    const segments = [];
    let count = 0;
    const appendClaim = (referenceKind, claimId, entityId) => {
    ...
    appendClaim('method-source-claim', observation.method.sourceClaimId, observation.method.entityId);
    appendClaim('dataset-source-claim', observation.dataset.sourceClaimId, observation.dataset.entityId);
    appendClaim('metric-source-claim', observation.metric.sourceClaimId, observation.metric.entityId);
    ...
    if (observation.evaluationProtocol.status === 'reported') {
        appendClaim('evaluation-protocol-source-claim', observation.evaluationProtocol.sourceClaimId);
    }
    if (observation.conditions.status === 'reported') {
        ...
        for (const [index, condition] of observation.conditions.values.entries()) {
            if (!activeClaims.has(condition.sourceClaimId))
    ...
    for (const [role, entityId] of entityReferences) {
        ...
        const claimIds = projectionIndexValue(staleSourceClaimIdsByEntity, entity.id, 'entity stale source claims');
...
function observationStaleReferenceAt(observation, targetIndex, referenceIndex) {
    ...
    for (const segment of referenceIndex.segments) {
        switch (segment.kind) {
            ...
            case 'condition-claims': {
                ...
                const conditionIndex = segment.conditionIndexes[index];
            ...
            case 'entity-source-claims': {
                ...
                return {
                    ...
                    claim_id: segment.claimIds[index],
                    entity_id: segment.entityId,
                };
            }
...
function staleObservationReferenceAt(ranges, targetIndex, state) {
    const { range, index } = indexedReferenceRange(ranges, targetIndex, 'stale observation reference');
    const referenceIndex = projectionIndexValue(state.staleReferenceIndexByObservation, range.observation.id, 'observation stale reference index');
    return observationStaleReferenceAt(range.observation, index, referenceIndex);
}
function indexedReferenceRange(ranges, targetIndex, label) {
    let lower = 0;
    let upper = ranges.length - 1;
    while (lower < upper) {
        const middle = Math.floor((lower + upper + 1) / 2);
        const range = ranges[middle];
        if (range.start <= targetIndex)
            lower = middle;
        else
            upper = middle - 1;
    }
...
function staleSynthesisReferenceAt(ranges, targetIndex) {
    const { range, index } = indexedReferenceRange(ranges, targetIndex, 'stale synthesis reference');
    return {
        synthesis_id: range.synthesisId,
        finding_id: range.findingId,
        claim_id: range.staleClaimIds[index],
    };
}
function staleSynthesisComparisonReferenceAt(ranges, targetIndex, state) {
    const { range, index } = indexedReferenceRange(ranges, targetIndex, 'stale synthesis comparison reference');
...
function staleEntityReferenceAt(ranges, targetIndex) {
    const { range, index } = indexedReferenceRange(ranges, targetIndex, 'stale entity reference');
    return {
        entity_id: range.entityId,
        claim_id: range.staleClaimIds[index],
    };
}
function projectSynthesis(synthesis, active, budget, findingOffset) {
...
function projectMatrix(ctx, record, budget, offset, maximum) {
    ...
    const projectedCells = cells.values.map(({ facet, paperId }) => {
        ...
        const evidenceIds = uniqueSorted(matching.flatMap(claim => claim.evidenceLinks
            .filter(link => record.evidence.some(evidence => evidence.id === link.evidenceId && evidence.paperId === paperId))
...
function projectAudit(ctx, record, observationState, budget, offset, maximum) {
    ...
    for (const value of record.evidence)
        counts[evidenceCoverage(ctx, value)]++;
    ...
    const uncitedInferenceClaimIds = record.claims
        ...
        .map(value => value.id);
    const staleSynthesisReferenceRanges = [];
    ...
    const staleSynthesisComparisonReferenceRanges = [];
    let staleSynthesisComparisonReferenceTotal = 0;
    for (const synthesis of record.syntheses) {
        for (const finding of synthesis.findings) {
            const comparisonProtocolIds = finding.comparisonProtocolIds.filter(protocolId => !observationState.activeComparisonProtocols.has(protocolId)
                || observationState.staleComparisonProtocolIds.has(protocolId));
            if (comparisonProtocolIds.length === 0)
                continue;
    ...
    const staleEntityReferenceRanges = [];
    ...
    const staleObservationReferenceRanges = [];
    let staleObservationReferenceTotal = 0;
    for (const observation of record.observations) {
        if (!observationState.activeObservations.has(observation.id))
            continue;
        const referenceIndex = projectionIndexValue(observationState.staleReferenceIndexByObservation, observation.id, 'observation stale reference index');
        if (referenceIndex.count === 0)
            continue;
    ...
    const unprotocolledObservationIds = record.observations
        ...
        && projectionIndexValue(observationState.protocolsByObservation, observation.id, 'observation protocols').length === 0)
    ...
    const observationAlignmentBlockers = record.observations
        ...
        .map(observation => ({
        observation_id: observation.id,
        blockers: projectionIndexValue(observationState.blockersByObservation, observation.id, 'observation blockers'),
    }))
    ...
    const stale = budget.indexedReferences(staleSynthesisReferenceTotal, index => staleSynthesisReferenceAt(staleSynthesisReferenceRanges, index));
    const staleSynthesisComparisons = budget.indexedReferences(staleSynthesisComparisonReferenceTotal, index => staleSynthesisComparisonReferenceAt(staleSynthesisComparisonReferenceRanges, index, observationState));
    const staleEntities = budget.indexedReferences(staleEntityReferenceTotal, index => staleEntityReferenceAt(staleEntityReferenceRanges, index));
    const staleObservations = budget.indexedReferences(staleObservationReferenceTotal, index => staleObservationReferenceAt(staleObservationReferenceRanges, index, observationState));
...
function evidenceCoverage(ctx, evidence) {
    ...
    if (source === undefined || !observes(source, document))
        return 'parser-mismatch';
    const block = findBlock(document, evidence.locator.blockId);
    if (block === undefined || !sameLocatorAndText(block, evidence))
        return 'locator-mismatch';
    return 'readable';
}
function observes(source, document) {
    return source.observations.some(value => value.parserId === document.parser.id && value.parserVersion === document.parser.version);
}
function sameLocatorAndText(block, evidence) {
...
function exactSelection(quote, blockText) {
    if (quote.length === 0)
        throw new Error('quote must not be empty');
    const first = blockText.indexOf(quote);
    if (first === -1)
        return { status: 'quote-not-found', matches: 0 };
    let matches = 0;
    let cursor = 0;
...
function activeClaimIds(record) {
...
function activeEntityIds(record) {
...
function activeObservationIds(record) {
    const superseded = new Set(record.observations.flatMap(value => value.supersedes === undefined ? [] : [value.supersedes]));
    return new Set(record.observations
        .filter(value => !superseded.has(value.id))
        .map(value => value.id));
}
function activeComparisonProtocolIds(record) {
    const superseded = new Set(record.comparisonProtocols.flatMap(value => value.supersedes === undefined ? [] : [value.supersedes]));
...
function activeReadingNoteIds(record) {
    const superseded = new Set(record.readingNotes.flatMap(value => value.supersedes === undefined ? [] : [value.supersedes]));
    return new Set(record.readingNotes.filter(value => !superseded.has(value.id)).map(value => value.id));
}
function activeSynthesisIds(record) {
    const superseded = new Set(record.syntheses.flatMap(value => value.supersedes === undefined ? [] : [value.supersedes]));
    return new Set(record.syntheses.filter(value => !superseded.has(value.id)).map(value => value.id));
}
function paperTitleOf(ctx, paperId) {
    return ctx.researchLibrary.get(paperId)?.metadata.title.value ?? `[missing paper ${paperId}]`;
}
function agentAuthor(exec) {
    if (exec.agent === undefined)
        throw new Error('research-information writes require an owning agent session');
    return { kind: 'agent', id: ResearchAuthorId(String(exec.agent.id)) };
}
function pageItems(values, offset, maximum) {
...
function pageText(value, offset, maximumCodeUnits) {
    ...
    for (let codeUnitIndex = 0; codeUnitIndex < value.length;) {
        ...
        if (codePoint === undefined)
            throw new Error('tool-research-information: invalid text paging index');
...
function noteTextRenderCapacity(value, offset, maximum, noteId, evidenceId) {
...
function decimalTextRenderCapacity(value, offset, maximum, observationId, field) {
    let capacity = maximum;
    for (;;) {
        const page = pageText(value, offset, capacity);
...
function projectReviewWarning(value) {
    return {
        code: value.code,
...
function reviewTextRenderCapacity(markdown, offset, maximum, status, renderDigest) {
    let capacity = maximum;
    for (;;) {
...
function formatReviewRender(value) {
...
function formatReviewPage(page, status, renderDigest) {
...
class ProjectionBudget {
    ...
    referencesTotal = 0;
    truncated = false;
    constructor(config, referenceOffset = 0) {
    ...
    pagedText(value, offset, maximum) {
    ...
    references(values) {
        return this.indexedReferences(values.length, index => values[index]);
    }
    indexedReferences(total, valueAt) {
...
function codePointSafePrefix(value, maximumCodeUnits) {
    let end = Math.min(value.length, maximumCodeUnits);
    if ((value.charCodeAt(end - 1) & 0xFC00) === 0xD800)
        end--;
...
function parseReadingNoteId(value) {
    return ResearchReadingNoteId(uuid('supersedes_note_id', value));
}
function parseClaimId(value, field = 'claim_id') {
    return ResearchClaimId(uuid(field, value));
}
function parseEntityId(value, field = 'entity_id') {
...
function parseComparisonProtocolId(value) {
...
function parseSynthesisId(value) {
    return ResearchSynthesisId(uuid('synthesis_id', value));
}
function parseOtherMethodRole(role, otherRole) {
    if (role === 'other') {
        if (otherRole === undefined)
            throw new Error('method.other_role is required when method.role=other');
        return { otherRole };
    }
    if (otherRole !== undefined) {
...
function parseReportedContext(context, field) {
    if (context.status === 'reported') {
...
function parseObservationUnit(unit) {
    if (unit.status === 'reported') {
        if (unit.symbol === undefined)
            throw new Error('unit.symbol is required when unit.status=reported');
        return { status: unit.status, symbol: unit.symbol };
    }
    if (unit.symbol !== undefined)
        throw new Error('unit.symbol is valid only when unit.status=reported');
    return { status: unit.status };
}
function parseObservationUncertainty(uncertainty) {
    ...
    if (kind === undefined)
        throw new Error('uncertainty.kind is required when uncertainty.status=reported');
...
function normalizedQuery(value, maximum, name = 'query') {
...
function projectionIndexValue(values, key, description) {
...
function resolveConfig(config = {}) {
    const resolved = {
        ...
        maxItemsPerResult: positiveSafeInteger('maxItemsPerResult', config.maxItemsPerResult ?? DEFAULT_MAX_ITEMS_PER_RESULT),
        maxReferencesPerResult: positiveSafeInteger('maxReferencesPerResult', config.maxReferencesPerResult ?? DEFAULT_MAX_REFERENCES_PER_RESULT),
        ...
        maxReviewTextChars: positiveSafeInteger('maxReviewTextChars', config.maxReviewTextChars ?? DEFAULT_MAX_REVIEW_TEXT_CHARS),
    };
...
function formatResearchQuestionGet(value, maximum) {
    const regular = `Research question\n${JSON.stringify(value, null, 2)}`;
    if (regular.length <= maximum)
        return regular;
    const output = value;
...
function formatNoteTextPage(page, noteId, evidenceId) {
    const continuation = page.nextOffset === undefined
        ? '\nnext_text_offset=none'
...
function formatDecimalTextPage(page, observationId, field) {
    const continuation = page.nextOffset === undefined
...
function parseRenderDigest(value) {
    if (!/^sha256:[0-9a-f]{64}$/u.test(value)) {
