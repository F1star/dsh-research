...
import { researchLibraryDomainSpec } from "./spec.js";
export { researchLibraryDomainSpec, researchPaperRecord } from "./spec.js";
/**
 * Brand a validated or generated paper id.
 * @param value - Raw paper id string.
 * @returns the same string with its paper-id brand.
 */
export function ResearchPaperId(value) {
    return value;
}
/**
 ...
 */
export function ResearchSourceVersionId(value) {
    return value;
}
...
export class ResearchLibrary extends Service {
    ...
    constructor(ctx, config = {}) {
        ...
        this.config = {
            ...
            maxSourceVersionsPerPaper: positiveSafeInteger('maxSourceVersionsPerPaper', config.maxSourceVersionsPerPaper ?? DEFAULT_MAX_SOURCE_VERSIONS_PER_PAPER),
            maxObservationsPerSource: positiveSafeInteger('maxObservationsPerSource', config.maxObservationsPerSource ?? DEFAULT_MAX_OBSERVATIONS_PER_SOURCE),
            maxExternalIdsPerPaper: positiveSafeInteger('maxExternalIdsPerPaper', config.maxExternalIdsPerPaper ?? DEFAULT_MAX_EXTERNAL_IDS_PER_PAPER),
            maxAliasesPerSource: positiveSafeInteger('maxAliasesPerSource', config.maxAliasesPerSource ?? DEFAULT_MAX_ALIASES_PER_SOURCE),
    ...
    async registerNow(request) {
        const table = this.requireTable();
        const externalHolders = new Map();
        ...
        if (request.paperId !== undefined) {
            ...
            const exactHolders = new Set(externalHolders.keys());
            if (documentHolder !== undefined)
                exactHolders.add(documentHolder.id);
            if (exactHolders.size > 1) {
                const identifiers = [...externalHolders.values()].flat();
                if (request.document !== undefined)
                    identifiers.push(`document:${request.document.documentId}`);
                return {
                    status: 'identifier-conflict',
            ...
            if (selected !== undefined)
                target = table.get(selected);
        }
        if (target === undefined)
            return await this.createPaper(request);
        return await this.enrichPaper(target, request);
    }
    async createPaper(request) {
        const table = this.requireTable();
        if (table.size >= this.config.maxPapers)
            return capacity('papers');
        if (request.externalIds.length > this.config.maxExternalIdsPerPaper)
            return capacity('external-ids');
    ...
    async enrichPaper(current, request) {
        ...
        for (const identifier of request.externalIds) {
            if (externalIds.some(value => sameIdentifier(value, identifier)))
                continue;
            if (externalIds.length >= this.config.maxExternalIdsPerPaper)
                return capacity('external-ids');
        ...
        if (document !== undefined) {
            ...
            if (sourceIndex === -1) {
                if (sourceVersions.length >= this.config.maxSourceVersionsPerPaper)
                    return capacity('source-versions');
                ...
                const source = sourceVersions[sourceIndex];
                sourceVersionId = source.id;
                const observationIndex = source.observations.findIndex(observation => observation.parserId === document.parserId
                    && observation.parserVersion === document.parserVersion);
                let observations = source.observations;
                if (observationIndex === -1) {
                    if (observations.length >= this.config.maxObservationsPerSource)
                        return capacity('observations');
                    observations = [...observations, observationFrom(document, now)];
                    changed = true;
                }
                else if (!sameObservation(observations[observationIndex], document)) {
                    return {
                ...
                if (document.sourceAlias !== undefined
                    ...
                    if (aliases.length >= this.config.maxAliasesPerSource)
                        return capacity('source-aliases');
                    aliases = [...aliases, {
    ...
    async updateAliasNow(request) {
        ...
        if (request.kind !== 'source') {
            ...
            if (request.action === 'remove') {
                if (index === -1)
                    return { status: 'unchanged', paper: current };
            ...
            if (current.externalIds.length >= this.config.maxExternalIdsPerPaper)
                return capacity('external-ids');
            const externalIds = [...current.externalIds, {
                    kind: request.kind,
                    value: request.value,
        ...
        const source = current.sourceVersions[sourceIndex];
    ...
    normalizeText(field, raw) {
        const value = raw.normalize('NFKC').trim().replace(/\s+/gu, ' ');
        if (value.length === 0)
            throw new Error(`research-library ${field} must be non-empty`);
        this.assertFieldBytes(field, value);
        return value;
    }
    assertFieldBytes(field, value) {
        if (Buffer.byteLength(value, 'utf8') > this.config.maxFieldBytes) {
            throw new Error(`research-library ${field} exceeds configured UTF-8 limit ${this.config.maxFieldBytes}`);
        }
    }
    validateStoredState() {
        const table = this.requireTable();
    ...
    validateRecord(record) {
        ...
        if (record.metadata.authors !== undefined) {
            ...
            for (const author of record.metadata.authors.value)
                this.assertCanonicalText('author', author);
        }
        if (record.metadata.venue !== undefined)
            this.assertCanonicalText('venue', record.metadata.venue.value);
        ...
        this.assertUniqueStrings(`paper '${record.id}' external ids`, record.externalIds.map(identifier => `${identifier.kind}:${identifier.value}`));
        for (const identifier of record.externalIds) {
            const normalized = this.normalizeIdentifier(identifier.kind, identifier.value);
            if (normalized !== identifier.value)
                throw inconsistent(`paper '${record.id}' stores non-canonical ${identifier.kind}`);
        }
        ...
        if (record.updatedAt < record.createdAt)
            throw inconsistent(`paper '${record.id}' updatedAt precedes createdAt`);
        for (const source of record.sourceVersions)
            this.validateSource(record.id, source);
    }
    validateSource(paperId, source) {
        if (source.aliases.length > this.config.maxAliasesPerSource) {
        ...
        for (const alias of source.aliases)
            this.assertCanonicalText('source alias', alias.value);
        ...
        this.assertUniqueStrings(`source '${source.id}' parser revisions`, source.observations.map(observation => `${observation.parserId}\u0000${observation.parserVersion}`));
        for (const observation of source.observations) {
            ...
            if (observation.documentTitle !== undefined)
                this.assertCanonicalText('document title', observation.documentTitle);
        }
        if (source.updatedAt < source.createdAt)
            throw inconsistent(`source '${source.id}' updatedAt precedes createdAt`);
    }
    ...
    assertUniqueStrings(subject, values) {
        if (new Set(values).size !== values.length)
            throw inconsistent(`${subject} contain duplicates`);
    }
    requireTable() {
        if (this.table === undefined)
            throw new Error('research library is not started yet');
        return this.table;
    }
...
function observationFrom(document, now) {
    return {
...
function sameObservation(observation, document) {
...
function metadataConflicts(current, supplied) {
    if (supplied === undefined)
        return [];
    const conflicts = [];
    if (supplied.title !== undefined && supplied.title !== current.title.value)
        conflicts.push('title');
    if (supplied.authors !== undefined && current.authors !== undefined
        && !sameStrings(supplied.authors, current.authors.value))
...
function fillMetadata(current, supplied, changed) {
    if (supplied === undefined)
        return current;
...
function changeValue(value, changed) {
    changed();
    return value;
}
...
function deduplicateIdentifiers(values) {
    const seen = new Set();
...
function capacity(resource) {
    return { status: 'capacity', resource };
}
function instant() {
