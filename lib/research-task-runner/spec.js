...
const runId = z.uuid().transform(value => value);
...
export const researchTaskRunSchema = z.strictObject({
    id: runId, taskId, taskRevision: integer,
    sessionId: text.transform(value => value), agentPreset: text,
    model: z.strictObject({ provider: text, model: text,
        reasoningEffort: text.transform(value => value).optional() }),
...
export const startResearchTaskRunSchema = z.strictObject({
...
export const stopResearchTaskRunSchema = z.strictObject({ runId, author });
...
export const researchTaskRunDomainSpec = defineDomain({ name: 'research_task_run', version: 1,
    tables: { runs: domainTable(researchTaskRunSchema) } });
